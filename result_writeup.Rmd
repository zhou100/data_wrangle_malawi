...
plot_long= dplyr::bind_rows(plot_long, long_FCS_hh)

p = ggplot(as.data.frame(plot_long),aes(x=logFCS,color=level)) + geom_density(alpha=0.1) +xlim(2.8, 4.9) +
...
plot_long= dplyr::bind_rows(plot_long, long_HDDS_hh)


p= ggplot(as.data.frame(plot_long),aes(x=HDDS, color=level))+ geom_density(alpha=0.25,show.legend = TRUE) +
...
plot_long= dplyr::bind_rows(plot_long, long_RCSI_hh)


p = ggplot(as.data.frame(plot_long),aes(x=RCSI, color=level))+ stat_density(data=plot_long_ipc,aes(x=RCSI,y=..scaled../2.3,color=level)) + geom_density(alpha=0.25)+xlim(0, 45) + 
...
r2matlist<-lapply(r2matlist,tibble::rownames_to_column)



r2mat_FCS %>% tidyr::gather(value = model )
``` 
