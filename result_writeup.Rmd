...
p<-ggplot(as.data.frame(long_data[long_data$measure!="logFCS",]),aes(x=months,color=measure,y=Food_security,shape = measure,group=measure)) + theme_bw() + geom_path(size=2.5)+ geom_point(size=5)  
# adding the relative humidity data, transformed to match roughly the range of the temperature
p <- p +geom_line(size=2.5,data=FCS_long,aes(y = Food_security*1.1, colour = measure))  + geom_point(data=FCS_long,aes(y = Food_security*1.1, colour = measure),size=5)  
...
ggplot(as.data.frame(rcsi_pair), aes(actual, predict )) +
  ...
  annotate(geom = "text", size = 5, x = 11, y = 1.5, label = paste("R Squared= ",r2list[1],sep = " ")) +
...
ggplot(as.data.frame(logFCS_pair), aes(actual, predict)) +
  ...
  theme_minimal() + geom_abline(intercept = 0, slope = 1)+
  scale_color_brewer(palette="Dark2") +
  annotate(geom = "text", size = 5, x = 4.28, y = 3, label = paste("R Squared= ",r2list[2],sep = " ")) +
...
ggplot(as.data.frame(HDDS_pair), aes(actual, predict)) +
  ...
  theme_minimal() + geom_abline(intercept = 0, slope = 1)+
  scale_color_brewer(palette="Dark2")+
  annotate(geom = "text", size = 5, x = 6.5, y = 4.5, label = paste("R Squared= ",r2list[3],sep = " "))+
...
ggplot(as.data.frame(rcsi_pair), aes(actual, predict )) +
  ...
  geom_line(aes( x, y, linetype = cutoff), cutoff3) +
  geom_line(aes( x, y, linetype = cutoff), cutoff4) +
  geom_point(data = as.data.frame(RCSI_IPC),aes(clust_RCSI, clust_RCSI_predict_ipc),shape = 1,size =6,show.legend = FALSE,na.rm = TRUE) +
...
d.	 R Squaredof 2013 predication 
